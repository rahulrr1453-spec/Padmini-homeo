...
import { LayoutDashboard, Users, FileText, Calendar, Pill, Sun, Moon, ShieldAlert } from 'lucide-react';

const Sidebar = ({ activeTab, setActiveTab, isDarkMode, toggleDarkMode, doctors, activeDoctor, setActiveDoctor }) => {
  const navItems = [
    ...
    <div className="w-64 bg-emerald-900 text-white min-h-screen shadow-xl flex flex-col animate-slide-in relative z-10">
      ...
      {doctors && doctors.length > 0 && (
        <div className="px-4 mb-4">
          ...
          <select 
            className="w-full bg-emerald-800/50 text-white border border-emerald-700/50 rounded-lg p-2 text-sm focus:ring-2 focus:ring-emerald-500 outline-none"
      ...
      <nav className="flex-1 px-4 space-y-2">
        {navItems.map((item) => (
          <button
            ...
            onClick={() => {
              if (item.id === 'admin') {
                const key = window.prompt('Enter Admin Secret Key:');
                if (key === 'padmini@vittal') {
